var AddressInput = React.createClass({
	onChange: function(e) {
		...
		if(this.props.parentOnAddressChange) {
			this.props.parentOnAddressChange(address.zipCode, address.city, address.full);
		}
	},
	getAddress: function() {
		var address = {};
		var town = this.refs.town.value;
		address.zipCode = '';
		address.city = this.refs.county.value;
		if(('縣市' != address.city) && ('鄉鎮市區' != town)) {
			address.zipCode = this.postcodeTable[address.city][town];
		}
		var remain = this.refs.remain.value;
		address.remain = remain;
		address.full = remain;
        if(address.zipCode) {
            address.full = "(" + address.zipCode + ")" + address.city + town + remain;
        }
		return address;
	},
	render: function() {
		...
		var address = {};
		address.zipCode = this.props.zipCode;
		address.city = this.props.city || '縣市';
		address.address = this.props.address;

        var zipCodeRexExp = new RegExp('\(([0-9]+)\)');
        var zipCodeMatches = zipCodeRexExp.exec(address.address);
        if(!!zipCodeMatches) {
            address.zipCode = zipCodeMatches[1];
            for(var county in this.postcodeTable) {
                for(var town in this.postcodeTable[county]) {
                    if(address.zipCode === this.postcodeTable[county][town]) {
                        address.city = county;
                    }
                }
            }
		...
		var countyArray = [];
		for(var county in this.postcodeTable) { countyArray.push(county); }

		var usingTownArray = [];
		address.town = '鄉鎮市區';
		for(var town in this.postcodeTable[address.city]) {
			if(-1 != address.address.indexOf(town)) { address.town = town; }
			usingTownArray.push({
				townName: town, postCode: this.postcodeTable[address.city][town]
			});
		}
		address.remain = '';
		if('鄉鎮市區' != address.town) {
			var town = address.town;
			var remainStart = address.address.indexOf(address.town) + address.town.length;
			address.remain = address.address.slice(remainStart, address.address.length);
		}
        if(!address.remain) { address.remain = address.address; }
		return <span className='address-input'>
            <select
                className="address-city-selector" ref="county"
                disabled={disabled} value={address.city} onChange={this.onChange}
            >
            ...
            <select
                className="address-town-selector" ref="town"
                disabled={disabled} value={address.town} onChange={this.onChange}
            >
                {(address.city === '縣市') && <option value="鄉鎮市區">鄉鎮市區</option>}
                {usingTownArray.map((townPair) => {
                    return <option value={townPair.townName} key={townPair.townName}>
                        {'(' + townPair.postCode + ') ' + townPair.townName}
                    </option>
                })}
            </select>
            <input
                name="people.address" ref="remain"
                className="address-remain-input" size="50" type="text" placeholder='請先選擇縣市'
                disabled={disabled} value={address.remain} onChange={this.onChange}
            />
        </span>;
	},
	/**
